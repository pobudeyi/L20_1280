...
#define I2C_SDA_INPUT() 			{GPIOB->MODER &= 0xFFFF3fff;}//PB9 input
#define I2C_SDA_OUTPUT()		 	{GPIOB->MODER &= 0xFFFF3fff;  GPIOB->MODER |= 0x00004000;}
	#endif
	#if defined(USE_I2C_1_PB8_9)
	#define I2C_SCL_INPUT()
	#define I2C_SCL_OUTPUT()
	#define I2C_SDA_INPUT()			{GPIOB->MODER&=~(3<<(9*2));GPIOB->MODER|=0<<9*2;}//PB9 input
	#define I2C_SDA_OUTPUT() 		 {GPIOB->MODER&=~(3<<(9*2));GPIOB->MODER|=1<<9*2;}//PB9 output
	#endif
	#define I2C_SCL_1()					HAL_GPIO_WritePin(GPIOB,I2C_SCL_PIN,GPIO_PIN_SET)		//GPIO_PORT_I2C->BSRR = I2C_SCL_PIN				/* SCL = 1 */
	...
	#define I2C_SDA_READ()				HAL_GPIO_ReadPin(GPIOB, I2C_SDA_PIN)	/* 读SDA口线状态 */
#endif
