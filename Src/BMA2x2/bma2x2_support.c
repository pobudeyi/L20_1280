...
/************************* END INITIALIZATION *************************/
	bma2x2_set_intr_output_type(BMA2x2_INTR1_OUTPUT,PUSS_PULL);	//Push-pull
	bma2x2_set_thres(BMA2x2_SLOPE_THRES,40);
	...
	bma2x2_set_source(BMA2x2_SOURCE_SLOPE,INTR_ENABLE);
...
struct bma2x2_accel_eight_resolution sensor_pos={0};

void print_accel(void)
{
    SYS_DEBUG("x:%d,y:%d,c:%d\r\n",sensor_pos.x,sensor_pos.y,sensor_pos.z);
}

int bma2x2_get_accel(void)
{
    return bma2x2_read_accel_eight_resolution_xyz(&sensor_pos);
}

s32 bma2x2_open(void)
{
...
/*------------------------------------------------------------------*/
    ...
    SYS_DEBUG("sensor init:%d/r/n",com_rslt);
    return com_rslt;
}
...
//{
//    s32 com_rslt = BMA2x2_ERROR;
//    u8 *power_mode_u8 = BMA2x2_MODE_STANDBY;
//
//    com_rslt += bma2x2_get_power_mode(&power_mode_u8);
//
//    if(BMA2x2_MODE_NORMAL == power_mode_u8)
//        return;
//
//    com_rslt += bma2x2_set_power_mode(BMA2x2_MODE_NORMAL);
//
...
s32 bma2x2_lowpower1(void)
{
    ...
    if(BMA2x2_MODE_LOWPOWER1 == power_mode_u8)
        return BMA2x2_SUCCESS;

    com_rslt += bma2x2_set_power_mode(BMA2x2_MODE_LOWPOWER1);

    return com_rslt;
}

s32 bma2x2_lowpower2(void)
{
    s32 com_rslt = BMA2x2_ERROR;
    u8 power_mode_u8 = BMA2x2_MODE_STANDBY;
    ...
    if(BMA2x2_MODE_LOWPOWER2 == power_mode_u8)
        return BMA2x2_SUCCESS;

    com_rslt += bma2x2_set_power_mode(BMA2x2_MODE_LOWPOWER2);

    return com_rslt;
}

s32 bma2x2_suspend(void)
{
    s32 com_rslt = BMA2x2_ERROR;
    u8 power_mode_u8 = BMA2x2_MODE_STANDBY;
    ...
    if(BMA2x2_MODE_SUSPEND == power_mode_u8)
        return BMA2x2_SUCCESS;

    com_rslt += bma2x2_set_power_mode(BMA2x2_MODE_SUSPEND);

    return com_rslt;
}

s32 bma2x2_deep_suspend(void)
{
    s32 com_rslt = BMA2x2_ERROR;
    u8 power_mode_u8 = BMA2x2_MODE_STANDBY;
    ...
    if(BMA2x2_MODE_DEEP_SUSPEND == power_mode_u8)
        return BMA2x2_SUCCESS;
...
s32 bma2x2_standby(void)
{
    s32 com_rslt = BMA2x2_ERROR;
    u8 power_mode_u8 = BMA2x2_MODE_NORMAL;

    com_rslt += bma2x2_get_power_mode(&power_mode_u8);

    if(BMA2x2_MODE_STANDBY == power_mode_u8)
        return BMA2x2_SUCCESS;
...
s8 BMA2x2_I2C_bus_write(u8 dev_addr, u8 reg_addr, u8 *reg_data, u8 cnt)
{
	...
	* In the driver BMA2x2_SUCCESS defined as 0
	...
	*/
	if(Gsensor_Itf_Write((dev_addr << 1), reg_addr,reg_data,cnt,1000) == false)
	{
		iError = BMA2x2_ERROR;
	}
	return (s8)iError;
...
s8 BMA2x2_I2C_bus_read(u8 dev_addr, u8 reg_addr, u8 *reg_data, u8 cnt)
{
     ...
     * and FAILURE defined as -1
	 */
	if(Gsensor_Itf_Read((dev_addr <<1), reg_addr,reg_data,cnt,1000) == false)
	{
		iError = BMA2x2_ERROR;
	}
